...
We will start the mathematical exploration of this blog with a small introduction to spectral sequences before we delve into the main results in Odins master thesis in some later post.
...
![On the first page we see some of the $d_1$ differentials. Note that the differentials go one to the left. One example of a vanishing differential is the map $E_{0,0}^1 \to E_{-1,0}^1$, which vanishes as $E_{-1,0}^1$ is zero in this first quadrant spectral sequence, as indicated by a dark patch.](images/SpecSeqd1.png)
...
To be kind off vague, we can set up the homologies after each other, with the respective homologies of the quotient spaces.
...
Now, if we take homology, what happens?
...
In this post, we have done a lot of the "dirty work" when it comes to understanding spectral sequences. We will probably see some applications of this in the next post, illustrating the beautiful methods of computing homology we have developed. 
Until next time, check out [this video](https://www.youtube.com/watch?v=OnvQggy3Ezw) on freediving. It recently blew my mind. 
